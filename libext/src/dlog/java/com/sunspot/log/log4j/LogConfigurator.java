...
public class LogConfigurator {
    ...
    public static final String LOG_FILE_PATTERN_SIGNEL = "%d: %c: %m%n";

    /**
     * %d：时间、%t:线程、 %m：日志信息、%n：回车换行符
     */
    public static final String LOG_FILE_PATTERN_MULTI = "%d --> [%t]: %m%n";

    private LogConfigurator() {
        this.rootLevel = Level.DEBUG;
        this.filePattern = LOG_FILE_PATTERN_SIGNEL;
        this.logCatPattern = "%m%n";
        this.fileName = "live-records.log";
    ...
    public void configure(Logger logger) {
        ...
        boolean fileAppend = true; //偶现添加文件appender异常，则过滤本次添加，下次打印时重试添加
    ...
    private boolean configureFileAppender(Logger logger) {
        ...
        try {
            ...
            try {//amos.modify.2016-05-09 修复友盟上bug：Failed to allocate a 32 byte allocation with 8 free bytes and 8B until OOM
        ...
        if (rollingFileAppender != null) {
            ...
            if (!this.isUseRootLogger()) {
                logger.setAdditivity(false); //不在root logger中打印日志
            } else {
